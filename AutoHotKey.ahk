...
; Run Ubuntu directly with F10, starting bash
^F10::
...
; Start terminator with CTRL-F10, if installed as described here: 
...
; Run Ubuntu directly with CTRL-F10, starting bash
^F10::
Process, Exist, ubuntu2004.exe 
If !ErrorLevel ; is not running
{
...
; Start terminator with F10, if installed as described here: 
; - https://medium.com/@bhupathy/install-terminator-on-windows-with-wsl-2826591d2156 
; - https://www.pofilo.fr/post/20190727-terminator/
; And setup with:
; - https://gist.github.com/Raneomik/202f5adb964723b16d14c3799d28e1e2
...
; Turn of mic when pressing F8
F8::  ; F8 is my chosen hotkey

;Control panel > Sound > Recording > Select headset > Properties > Levels > Change > Apply

SoundSet, +1, MASTER, mute, 12 ; 12 is my EliteBook internal mic id, determined by script running on Win+F5 declared below
SoundGet, master_mute12, , mute, 12

SoundSet, +1, MASTER, mute, 14 ; 14 is my MPow headset id when attached, determined by script running on Win+F5 declared below... 
SoundGet, master_mute14, , mute, 14

; Don't need tooltip. I have the LED on the F8 button on my Elitebook keyboard
; ToolTip, Mute Status New(12): %master_mute12% -headset(6): %master_mute6% ;use a tool tip at mouse pointer to show what state mic is after toggle
...
; https://www.autohotkey.com/boards/viewtopic.php?t=15509
; Get the lsit of equipement, to identify which numbers are changing when, for example, switching on and off mic
#F5::
SetBatchLines -1
